...
class BmPropAction extends CommonAction{
    ...
    public function edit() {
        ...
        $condition['id'] = $id;
        $vo = M('Prop')->where($condition)->find();
        $this->assign ( 'vo', $vo );
        $this->display ();
    }
    ...
    public function foreverdelete() {
        ...
        if (isset ( $id )) {
            ...
            $rel_data = M('Prop')->where($condition)->findAll();
            foreach($rel_data as $data)
            {
                $info[] = $data['name'];
            }
            if($info) $info = implode(",",$info);
            $list = M('Prop')->where ( $condition )->delete();
            //删除相关预览图
//				foreach($rel_data as $data)
//				{
...
//		}
        $m_config = load_auto_cache('m_config');
        $ticket_name = $m_config['ticket_name']!=''?$m_config['ticket_name']:'印票';
        $log_info = M('Prop')->where("id=".intval($data['id']))->getField("name");
        //开始验证有效性
    ...
    public function set_effect()
    {
        ...
        $id = intval($_REQUEST['id']);
        $ajax = intval($_REQUEST['ajax']);
        $info = M('Prop')->where("id=".$id)->getField("name");
        $c_is_effect = M('Prop')->where("id=".$id)->getField("is_effect");  //当前状态
        $n_is_effect = $c_is_effect == 0 ? 1 : 0; //需设置的状态
        M('Prop')->where("id=".$id)->setField("is_effect",$n_is_effect);
    ...
    public function set_sort()
    {
        ...
        $log_info = M('Prop')->where("id=".$id)->getField("name");
        if(!check_sort($sort))
        {
