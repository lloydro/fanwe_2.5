<?php
	class SocietyLevelAction extends CommonAction{
		 ...
		 */
		public function add(){
			$this->assign("new_level", M("SocietyLevel")->max("level")+1);
            $this->assign("old_score", M("SocietyLevel")->max("score"));
			$this->display();
		}
		/**
		 * 编辑界面
		...
		public function update(){
            ...
            $society_level_list = M(MODULE_NAME)->where("level=".intval($data['level']-1))->find();
			if($society_level_list)
			{
                if(intval($data['score'])<=$society_level_list['score']){
