...
var vm = avalon.define({
    ...
    login: function() {
        ...
        if(this.check()){
            console.log(11);
            let form_data = self.login_type == 0 ? { mobile: self.mobile, verify_coder: self.verify_coder, type: 0 } : { mobile: self.mobile, password: self.password, type: 1 };
            $.ajax({
                ...
                success: function(result) {
                    if (result.status == 1) {
                        ...
                        setTimeout(function(){
                            location.href = TMPL_REAL + "/index.php?ctl=user&act=promoter_info";
                        }, 1000); 
                    } else {
