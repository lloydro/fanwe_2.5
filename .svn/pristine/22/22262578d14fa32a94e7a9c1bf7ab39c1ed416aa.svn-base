...
var $checkAction = {
  ...
  */
	minLength: function minLength(value, length, isByte) {
		var strLength = value.trim().length;
		if (isByte) strLength = $objAction.getStringLength(value);
  ...
  */
	maxLength: function maxLength(value, length, isByte) {
		var strLength = value.trim().length;
		if (isByte) strLength = $objAction.getStringLength(value);
  ...
  */
	checkMobilePhone: function checkMobilePhone(value) {
		if (value.trim() != '') return (/^1[34578]\d{9}$/.test(value.trim())
		);else return false;
	},
  ...
  */
	checkint: function checkint(val) {
		return val % 1 === 0;
	},
	...
	checkEmpty: function checkEmpty(val) {
		switch (typeof val === 'undefined' ? 'undefined' : _typeof(val)) {
			case 'undefined':
				return true;
			case 'string':
				if ($.trim(val).length == 0) return true;break;
			case 'boolean':
				if (!val) return true;break;
			case 'number':
				if (0 === val) return true;break;
			case 'object':
  ...
  */
	checkID: function checkID(val) {
		if (val.trim() != '') return (/^(\d{15}$|^\d{18}$|^\d{17}(\d|X|x))$/.test(val.trim())
		);else return false;
	}
...
$sendVerifyCode = {
	...
	leftTimeFun: function leftTimeFun() {
		...
		if (self.code_lefttime > 0) {
			...
			self.code_timeer = setTimeout(function () {
				self.leftTimeFun();
			}, 1000);
		} else {
			self.code_lefttime = 60;
