...
//		$this->key .= $type;
		...
		if ($list === false) {
			...
			if(false){
				...
				if($page == 1){
					$weibo_info = $GLOBALS['db']->getRow("select w.user_id,w.id as weibo_id,u.head_image,u.is_authentication,w.content,w.red_count,w.unlike_count,w.digg_count,w.comment_count,w.video_count,w.data,u.v_icon,u.nick_name,w.sort_num,w.photo_image,u.city,w.is_top,w.price,w.type,w.create_time
...
//						}
					...
					//点赞列表
					$digg_list = $GLOBALS['db']->getAll("select wc.user_id,u.v_icon,u.nick_name,u.head_image,u.is_authentication from ".DB_PREFIX."weibo_comment as wc
            ...
            left join ".DB_PREFIX."user as u on wc.user_id = u.id where wc.weibo_id = ".$weibo_id." and type = 3 order by wc.comment_id desc limit 0,7");
				...
				//评论列表
				$comment_list = $GLOBALS['db']->getAll("select wc.comment_id, wc.user_id,u.v_icon,u.nick_name,u.head_image,wc.content,wc.to_comment_id,wc.to_user_id,wc.create_time,u.is_authentication from ".DB_PREFIX."weibo_comment as wc
            left join ".DB_PREFIX."user as u on wc.user_id = u.id where wc.weibo_id = ".$weibo_id." and type = 1 and is_del = 0 order by wc.comment_id desc limit $limit");
				if($comment_list){
