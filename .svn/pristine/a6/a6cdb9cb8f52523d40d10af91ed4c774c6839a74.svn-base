...
class UserNoticeAction extends CommonAction
{
    public function index()
    {
        ...
        foreach ($list as $k => &$v) {
            ...
            $list[$k]['take_name'] = emoji_decode(M("user")->where("id=" . $v['user_id'] . " ")->getField("nick_name")); //接收人
            $list[$k]['send_name'] = emoji_decode(M("user")->where("id=" . $v['send_id'] . " ")->getField("nick_name")); //发送人
    ...
    public function view_info()
    {
        ...
        if (isset($id)) {
            ...
            $list['take_name'] = emoji_decode(M("user")->where("id=" . $list['user_id'] . " ")->getField("nick_name")); //接收人
            $list['send_name'] = emoji_decode(M("user")->where("id=" . $list['send_id'] . " ")->getField("nick_name")); //发送人
            $this->assign("list", $list);
        }
        $this->display();
    }
