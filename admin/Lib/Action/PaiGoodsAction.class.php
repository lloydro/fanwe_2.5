...
class PaiGoodsAction extends CommonAction
{
    ...
    public function edit()
    {
        ...
        $vo['podcast_name'] = emoji_decode($vo['podcast_name']);
        $vo['last_user_name'] = emoji_decode($vo['last_user_name']);
        $vo['user_name'] = emoji_decode($vo['user_name']);
        if ($vo['is_true'] == 0) {
            $vo['is_true'] = "虚拟";
        } else {
            $vo['is_true'] = "实物";
    ...
    public function pai_record()
    {
        ...
        if ($count > 0) {
            ...
            foreach ($list as &$value) {
                $podcast_name = emoji_decode(M('pai_goods')->where(array('id' => $value['pai_id']))->field('podcast_name')->find());
                $user         = emoji_decode(M('user')->where(array('id' => $value['user_id']))->field('nick_name')->find());
    ...
    public function price_record()
    {
        ...
        if ($count > 0) {
            ...
            foreach ($info as $k => $v) {
                $info[$k]['user_name'] = emoji_decode($v['user_name']);
            }
        }
        $page = $p->show();
        $this->assign("page", $page);
    ...
    public function security_deposit_records()
    {
        ...
        foreach ($info as $key => $value) {
            $info[$key]['nick_name'] = emoji_decode($info[$key]['nick_name']);
        }
        $page = $p->show();
        $this->assign("page", $page);
        $this->assign("list", $info);
