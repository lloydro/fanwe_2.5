    ...
    public function set_ban($data)
    {
		...
		if($result){
            ...
            if($n_is_effect==0){
                ...
                $GLOBALS['db']->query("update ".DB_PREFIX."user set ban_type=0 where id=".$id);
            }
		}
    ...
    public function contribution_list($data){
        ...
        if($user)
        {
            ...
            foreach ($redis['list'] as $k => $v) {
                $redis['list'][$k]['nick_name'] = emoji_decode($v['nick_name']);
            }
...
//        }
        ...
        foreach($volist as $k=>$v){
            ...
            $volist[$k]['nick_name'] = emoji_decode($volist[$k]['nick_name']);
        }
